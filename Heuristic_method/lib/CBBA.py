...
class CBBA(object):
    ...
    time_window_flag: bool  # True if time window exists  时间窗是什么
    ...
    path_list: list  # 2D list 每个机器人的路径顺序
    times_list: list  # 2D list 每个机器人任务加入顺序
    scores_list: list  # 2D list 每个机器人记录的得分序列
    bid_list: list  # 2D list 机器人为每个任务的投标值 ？
    winners_list: list  # 2D list 获胜者列表
    winner_bid_list: list  # 2D list 获胜者投标值列表
    graph: list  # 2D list represents the structure of graph 机器人之间的拓扑结构
    AgentList: list  # 1D list, each entry is a dataclass Agent 机器人列表
    TaskList: list  # 1D list, each entry is a dataclass Task 任务列表
    ...
    def solve(self, AgentList: list, TaskList: list, WorldInfoInput: WorldInfo,
        ...
        while not done_flag:
            self.logger.info("now the iter is: %d", iter_idx)
            # 1. Communicate
                        ...
                        if old_z[k][j] == k:
                            ...
                            if z[i][j] == i:
                                if (old_y[k][j] - y[i][j]) > epsilon:  # Update
                                    z[i][j] = old_z[k][j]
                                    y[i][j] = old_y[k][j]
                                elif abs(old_y[k][j] - y[i][j]) <= epsilon:  # Equal scores 基本相等的分数 为了实现一致性 将两者融合
    ...
    def compute_bid(self, idx_agent: int, feasibility: list):
        ...
        for idx_task in range(self.num_tasks):
            # Check for compatibility between agent and task 计算任务和机器人之间的兼容性
            ...
            # this task is incompatible（不相容的） with my type
    ...
    def scoring_compute_score(self, idx_agent: int, task_current: Task, task_prev: Task,
        ...
        计算机器人按照当前分配方式该任务的得分 返回任务的期望开始时间
        """
        ...
        if (self.AgentList[idx_agent].agent_type == self.agent_types.index("quad")) or \
            ...
            # Compute score  任务时间窗 任务必须要在时间窗之内完成 ！！
            if self.time_window_flag:
                # if tasks have time window
